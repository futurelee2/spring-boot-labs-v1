...
public class AuthService {
    ...
    public UserResponse login(LoginRequest loginRequest) {
        ...
        // 매니저한테 검증해라고 지시 내림 ( 정보 : 로그인 request - 아이디, 비번)
        // 패턴에 맞게 UsernamePasswordAuthenticationToken로 사용해서 넘겨줌
         ...
         // holder > securitycontext -> authentication에 넣어줌

        return userService.getUserResponseByUsername(loginRequest.getUsername());
    }

    public void logout(HttpServletRequest request, HttpServletResponse response) {
        // TODO: SecurityContext에서 인증 정보를 제거하여 로그아웃 처리합니다.
        // 스프링한테 위임함
